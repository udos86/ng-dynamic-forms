...
module.exports = function (pkg, src, dest) {
    ...
    return function () {
        ...
        var versionNumber = Number(pkg.version.slice(-1)),
            versionString = /(\d\.\d\.\d-[a-z]+\.)\d{1}/,
            newVersionNumber = versionNumber + 1,
            newVersionString = pkg.version.replace(versionString, "$1" + newVersionNumber),
            versionField = /("version":\s)"\d\.\d\.\d-[a-z]+\.\d{1}"/,
            dependencyField = /("@ng2-dynamic-forms\/[a-z\-]+":\s)"\^\d\.\d\.\d-[a-z]+\.\d{1}"/g;
